...
public class RelativePositionConstraint implements Constraint {
    ...
    public RelativePositionConstraint(RigidBodyIndex rigidBodyIdA, RigidBodyIndex rigidBodyIdB, Vector3f localSpaceAnchorA, Vector3f localSpaceAnchorB, DriftParameters driftParameters, CachingModuleProvider cachingProvider) {
    ...
    public ConstraintData compute(RigidBodyIsland island) {
        return new RelativePositionConstraintData(rigidBodyIdA.getIdentifiedData(island), rigidBodyIdB.getIdentifiedData(island));
    }
    ...
    public class RelativePositionConstraintData implements ConstraintData {
        ...
        public StateConstraint[] getImpulses() {
            Matrix3x3 skewedRA = skew(derivedBodySpaceAnchorA.duplicate().negate());
            Matrix3x3 skewedRB = skew(derivedBodySpaceAnchorB.duplicate());
        ...
        public float[] getCorrections() {
            return new float[] {
                    derivedWorldSpaceAnchorA.getX() - derivedWorldSpaceAnchorB.getX(),
                    derivedWorldSpaceAnchorA.getY() - derivedWorldSpaceAnchorB.getY(),
                    derivedWorldSpaceAnchorA.getZ() - derivedWorldSpaceAnchorB.getZ()
            };
        }
        ...
        public StateConstraint[] getForces() {
            ...
            Matrix3x3 skewedROmegaA = skew(pointRotationalVelocityA.duplicate().negate());
            Matrix3x3 skewedROmegaB = skew(pointRotationalVelocityB.duplicate());
            return new StateConstraint[] {
                    new StateConstraint(new IdentifiedDynamicsData[] {indexA, indexB}, new VectorNf[]{
