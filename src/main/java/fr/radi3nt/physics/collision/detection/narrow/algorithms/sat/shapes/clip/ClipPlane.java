...
public class ClipPlane {
    ...
    public void clipUsingWorldSpace(TransformedObject planeTransform, ResultEdge[] otherEdges, int index) {
        Vector3f worldSpaceNormal = normal.duplicate();
        if (flipNormal)
            worldSpaceNormal.negate();
        planeTransform.getRotation().transform(worldSpaceNormal);

        float projectedClip = worldSpaceNormal.dot(planeTransform.toWorldSpace(vertexOnPlane));
    ...
    public Vector3f getVertexOnPlane() {
        return vertexOnPlane;
    }
}
